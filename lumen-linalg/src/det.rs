use crate::{linalg, FloatDType, Tensor, Result};

pub fn det<T: FloatDType>(a: &Tensor<T>) -> Result<T> {
...
pub fn rank<T: FloatDType>(a: &Tensor<T>, tol: T) -> Result<usize> {
...
fn det_permutation<T: FloatDType>(p: &Tensor<T>) -> Result<T> {
...
mod tests {
    ...
    fn test_det_random() -> Result<()> {
        let a = Tensor::<f64>::randn(0., 1.0, (5, 5))?;
        let det_a = det(&a)?;
        println!("{}", det_a);
        Ok(())
    }
