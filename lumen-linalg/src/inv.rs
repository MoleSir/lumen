use crate::{linalg, FloatDType, IndexOp, Tensor, Result};
...
pub fn inv<T: FloatDType>(a: &Tensor<T>, method: SolveMethod) -> Result<Tensor<T>> {
    check_square(a, "inv")?;
    let (n, _) = a.dims2()?;
...
/// 3. Return (L^-1)^T * L^-1
pub fn cholesky_inv<T: FloatDType>(a: &Tensor<T>) -> Result<Tensor<T>> {
...
/// 3. Return U^-1 * L^-1 * P
pub fn plu_inv<T: FloatDType>(a: &Tensor<T>) -> Result<Tensor<T>> {
...
/// 2. Compute R^-1
/// 3. Return R^-1 * Q^T
pub fn qr_inv<T: FloatDType>(a: &Tensor<T>) -> Result<Tensor<T>> {
...
pub fn lower_triangular_inv<T: FloatDType>(l: &Tensor<T>) -> Result<Tensor<T>> {
    check_square(l, "inv")?;
    let l = l.matrix_view_unsafe()?;
    let (n, _) = l.shape();

    let inv_arr = Tensor::<T>::zeros((n, n))?;
...
fn upper_triangular_inv<T: FloatDType>(u: &Tensor<T>) -> Result<Tensor<T>> {
...
mod tests {
    ...
    // 矩阵生成函数
    // -------------------------
    fn random_matrix(n: usize) -> Result<Tensor<f64>> {
        Tensor::<f64>::randn(0.0, 1.0, (n, n))
    }

    fn random_spd_matrix(n: usize) -> Result<Tensor<f64>> {
    ...
    fn diagonal_matrix(diag: &[f64]) -> Result<Tensor<f64>> {
        let n = diag.len();
        let mat = Tensor::<f64>::zeros((n, n))?;
    ...
    fn lower_triangular_matrix(n: usize) -> Result<Tensor<f64>> {
        let mat = Tensor::<f64>::zeros((n, n))?;
        let mut mat_view = mat.matrix_view_unsafe()?;
        for i in 0..n {
            for j in 0..=i {
    ...
    fn upper_triangular_matrix(n: usize) -> Result<Tensor<f64>> {
        let mat = Tensor::<f64>::zeros((n, n))?;
        let mut mat_view = mat.matrix_view_unsafe()?;
        for i in 0..n {
            for j in i..n {
    ...
    // 测试函数
    // -------------------------
    fn test_inverse(a: &Tensor<f64>, method: SolveMethod) -> Result<()> {
        let identity = Tensor::<f64>::eye(a.dims2()?.0)?;
