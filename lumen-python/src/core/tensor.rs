...
macro_rules! impl_arith_inplace_binary {
    ($lhs:ident, $rhs:ident, $method:ident) => {
        if let Ok(rhs) = $rhs.extract::<PyTensor>() {
            match (&$lhs.inner, &rhs.inner) {
...
impl PyTensor {
    ...
    pub fn detach(&self) -> PyResult<Self> {
        match &self.inner {
...
pub fn py_to_shape(obj: &Bound<'_, PyAny>) -> PyResult<Shape> {
    if obj.is_none() {
        Ok(().into())
...
pub fn to_value_error(e: impl std::error::Error) -> PyErr {
    PyValueError::new_err(e.to_string())
}

#[allow(unused)]
pub fn to_runtime_error(e: impl std::error::Error) -> PyErr {
