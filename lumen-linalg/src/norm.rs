use std::borrow::Cow;
use crate::{DTypeConvert, FloatCategory, FloatDType, IntCategory, IntDType, Tensor, NumCategory, NumDType};
...
/// Compute the specified norm of an Tensor
pub fn norm<T: NormOp>(arr: &Tensor<T>, norm: Norm) -> T::NormOutput {
    match norm {
...
pub fn f2_norm<T: NormOp>(arr: &Tensor<T>) -> T::NormOutput {
    T::frobenius_norm(arr)
}
...
pub trait NormOp: NumDType {
    ...
    fn lp_norm(arr: &Tensor<Self> , p: f64) -> Self::NormOutput;

    fn inf_norm(arr: &Tensor<Self>) -> Self::NormOutput;

    fn spectral_norm(arr: &Tensor<Self>) -> Self::NormOutput;

    fn nuclear_norm(arr: &Tensor<Self>) -> Self::NormOutput;

    fn l1_norm(arr: &Tensor<Self>) -> Self::NormOutput {
        Self::lp_norm(arr, 1.0)
    }

    fn l2_norm(arr: &Tensor<Self>) -> Self::NormOutput {
        Self::lp_norm(arr, 2.0)
    }

    fn frobenius_norm(arr: &Tensor<Self>) -> Self::NormOutput {
        Self::lp_norm(arr, 2.0)
    }
}
...
pub trait NormOpCategory<T: NumDType, C: NumCategory = <T as NumDType>::Category> {
    type NormOutput: FloatDType;
    fn lp_norm(arr: &Tensor<T>, p: f64) -> Self::NormOutput;
    fn inf_norm(arr: &Tensor<T>) -> Self::NormOutput;
    fn spectral_norm(arr: &Tensor<T>) -> Self::NormOutput;
    fn nuclear_norm(arr: &Tensor<T>) -> Self::NormOutput;
...
impl<T: IntDType + DTypeConvert<f64>> NormOpCategory<T, IntCategory> for T {
    ...
    fn lp_norm(arr: &Tensor<Self>, p: f64) -> f64 {
        let sum = arr.iter().map(|v| T::to_f64(v).abs().powf(p)).sum::<f64>();
        sum.powf(1. / p)
    }

    fn inf_norm(arr: &Tensor<Self>) -> f64 {
        T::to_f64(arr.iter().max().unwrap_or(T::zero()))
    }

    fn spectral_norm(arr: &Tensor<Self>) -> f64 {
        <f64 as NormOp>::spectral_norm(&arr.to_dtype::<f64>())
    }

    fn nuclear_norm(arr: &Tensor<Self>) -> f64 {
...
impl<T: FloatDType> NormOpCategory<T, FloatCategory> for T {
    ...
    fn lp_norm(arr: &Tensor<Self>, p: f64) -> T {
        let p = T::from_f64(p);
        let sum = arr.iter().map(|v| v.abs().powf(p)).sum::<T>();
        sum.powf(T::one() / p)
    }

    fn inf_norm(arr: &Tensor<Self>) -> T {
        arr.iter()
            .map(|v| v.abs())
            .fold(T::zero(), |acc, x| acc.max(x))
    }

    fn spectral_norm(arr: &Tensor<Self>) -> T {
    ...
    fn nuclear_norm(arr: &Tensor<Self>) -> T {
        if arr.element_count() == 0 {
            return T::zero();
        }
        if arr.rank() == 1 {
...
mod tests {
    ...
    fn test_int_empty_array() -> Result<()> {
        let a: Tensor<i32> = Tensor::zeros(0)?;
        let l1 = norm(&a, Norm::L1);
        assert_eq!(l1, 0.0);
