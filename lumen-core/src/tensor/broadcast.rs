...
mod tests {
    ...
    fn test_broadcast_add_scalar() {
        ...
        let b = Tensor::new(&[10.]).unwrap(); // 标量 (1,)
    ...
    fn test_broadcast_add_vector_to_matrix() {
        let a = Tensor::new(&[[1., 2., 3.], [4., 5., 6.]]).unwrap(); // (2,3)
        let b = Tensor::new(&[10., 20., 30.]).unwrap(); // (3,)
        let res = a.broadcast_add(&b).unwrap();
