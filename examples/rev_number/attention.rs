...
impl<T: FloatDType> Decoder<T> {
    pub fn new(output_size: usize, embed_size: usize, hidden_size: usize) -> anyhow::Result<Self> {
        ...
        let rnn = Lstm::new(embed_size, hidden_size, None)?;
        let fc_out = Linear::new(2 * hidden_size, output_size, true, None)?;
        let attention = Attention::new(hidden_size)?;
        
        Ok(Self { embedding, rnn, attention, fc_out })
    }
    ...
    pub fn forward(
        &self, 
        input_token: impl Into<IntTensor>, 
        ...
        // (batch, 1, hidden_size)
        let context = self.attention.forward(&output, &encoder_outputs).context("attention forward")?;

        // (batch, 1, hidden_size * 2)
        let concat_input = Tensor::cat(&[output, context], 2)?;

        // (batch_size, 1, hidden_size * 2) => (batch_size, 1, vocab_size)
        let prediction = self.fc_out.forward(&concat_input).context("linear forward")?;
...
#[derive(Module)]
pub struct Attention<T: FloatDType> {
    pub attn: Linear<T>,
}

impl<T: FloatDType> Attention<T> {
    pub fn new(hidden_size: usize) -> anyhow::Result<Self> {
        let attn = Linear::new(hidden_size, hidden_size, false, None).context("init attn")?;
        Ok(Self { attn })
    }
    ...
    /// 
    /// # Args
    /// - hidden (query): Decoder's hidden state (batch, 1, hidden_size)
    /// - encoder_outputs (key): Encoder outpus, (batch, seq_len, hidden_size)
    pub fn forward(&self, hidden: &Tensor<T>, encoder_outputs: &Tensor<T>) -> anyhow::Result<Tensor<T>> {
        // 1. attention scores
        // (batch, 1, hidden_size) => ((batch, 1, hidden_size))
        let energy = self.attn.forward(hidden)?;
        
        // (batch, 1, hidden_size) @ (batch, hidden_size, seq_len) => (batch, 1, seq_len)
        let scores = energy.matmul(&encoder_outputs.transpose_last()?)?;
        
        // 2. softmax
        // (batch, 1, seq_len) => (batch, 1, seq_len), weight for each atten output in `encoder_outputs`
        let attn_weights = lumen_nn::functional::softmax(&scores, 2)?;
    
        // 3. Context Vector
        // (batch, 1, seq_len) @ (batch, seq_len, hidden_size) = (batch, 1, hidden_size) 
        let context = attn_weights.matmul(&encoder_outputs)?;
    ...
    pub fn forward(&self, src: &Tensor<u32>, trg: Option<&Tensor<u32>>) -> anyhow::Result<Tensor<T>> {
        ...
        */
        ...
        for t in 0..seq_len + 1 {
            // (batch, 1, vocab_size)
            let (output, next_state) = self.decoder.forward(&input_token, &encoder_outputs, &state).context("decoder")?;
