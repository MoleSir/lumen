...
use crate::{Module, ModuleVisitor, ModuleVisitorMut, TensorVisitor, TensorVisitorMut};

#[derive(Clone)]
pub struct Buffer<T: FloatDType>(Tensor<T>);

impl<T: FloatDType> Deref for Buffer<T> {
...
impl<T: FloatDType> DerefMut for Buffer<T> {
    #[inline]
...
impl<T: FloatDType> Module<T> for Buffer<T> {
    #[inline]
    fn visit_param<Visitor: TensorVisitor<T>>(&self, _visitor: &mut Visitor) -> Result<(), Visitor::Error> {
        Ok(())
    }

    #[inline]
    fn visit_param_mut<Visitor: TensorVisitorMut<T>>(&mut self, _visitor: &mut Visitor) -> Result<(), Visitor::Error> {
        Ok(())
    }
